...
( function( root, factory ) {
    ...
    $.extend( calendar.prototype, {
        ...
        _initDate: function() {
            ...
            //将配置中的日期对象转化为YYYYMMDD形式
            config.minDate = config.minDate && dateToNum( config.minDate );
            config.maxDate = config.maxDate && dateToNum( config.maxDate );
            config.selectDate = config.selectDate && dateToNum( config.selectDate );
        ...
        _getDayList: function( year, month ) {
            ...
            for ( var i = 0; i < days; i++ ) {
                ...
                //不在指定范围内的日期置灰
                if ( ( config.minDate && dateNum < config.minDate ) ||
                    ( config.maxDate && dateNum > config.maxDate ) 
                ) {
                    jrClassName = 'dl iv';
